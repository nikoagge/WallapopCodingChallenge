...
class CharactersListViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
//        collectionView.register(CharactersListHeaderCollectionViewCell.self, forCellWithReuseIdentifier: CharactersListHeaderCollectionViewCell.identifier)
//        collectionView.register(CharactersListCollectionViewCell.self, forCellWithReuseIdentifier: CharactersListCollectionViewCell.identifier)
    ...
    func loadNextPage() {
        ...
        _page += 1
        loadingData = true
        let previousCount = characterViewModel?.charactersList.count ?? 0
        characterViewModel?.getCharacters(page: _page) { [weak self] (result) in
            ...
            case .Success(_, _):
                ...
                let count = self?.characterViewModel?.charactersList.count ?? 0
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return _isFirstLoading ? 1 : characterViewModel?.charactersList.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (indexPath.row >= (characterViewModel?.charactersList.count ?? 0) - preloadCount) && !loadingData {
            loadNextPage()
        }
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CharactersListCollectionViewCell.identifier, for: indexPath) as! CharactersListCollectionViewCell
        let characterModel = characterViewModel?.charactersList[indexPath.row]
        ...
        cell.nameView.backgroundColor = remanderBy4 == 1 || remanderBy4 == 2 ? .comicPinkColor : .comicBlueColor
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        case UICollectionView.elementKindSectionHeader:
            ...
            let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: CharactersListHeaderCollectionViewCell.identifier, for: indexPath) as! CharactersListHeaderCollectionViewCell
            header.rectangleView.backgroundColor = .comicYellowColor
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if !_isFirstLoading {
            if let character = characterViewModel?.charactersList[indexPath.row] {
