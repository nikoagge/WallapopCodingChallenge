...
class CharacterViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func loadNextPage() {
        ...
        let previousCount = characterViewModel?.comicsList.count
        characterViewModel?.getCharacterComics(page: _page, character: id){ [weak self] (result) in
            ...
            case .Success(_, _):
                ...
                let count = self?.characterViewModel?.comicsList.count ?? 0
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        return _isFirstLoading ? 0 : (characterViewModel?.comicsList.count ?? 0)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 0 {
            ...
            let count = characterViewModel?.comicsList.count ?? 0
            if (indexPath.row >= count - preloadCount) && !loadingData {
                loadNextPage()
            }
            let comicModel = characterViewModel?.comicsList[indexPath.row]
    ...
    private func characterCell(forCharacterModel characterModel: CharacterModel, at indexPath: IndexPath) -> CharacterCollectionViewCell {
        ...
        cell.nameView.backgroundColor = .comicYellowColor
        let url = URL(string: characterModel.thumbnail.fullName)
        cell.characterImageView.kf.setImage(with: url)
        return cell
    }

    private func descriptionCell(forCharacterModel characterModel: CharacterModel, at indexPath: IndexPath) -> DescriptionCollectionViewCell {
        ...
        cell.squareView.setBlackBorder()
        cell.squareView.backgroundColor = .comicYellowColor
    ...
    private func comicsTitleCell(withString string: String, at indexPath: IndexPath) -> ComicsTitleCollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ComicsTitleCollectionViewCell.identifier, for: indexPath) as! ComicsTitleCollectionViewCell
